...
export const GET = async (_req: NextRequest, { params }: Props) => {
  try {
    ...
    const at = await activityTypeService.getActivityType(id);
    ...
    if (error instanceof Error && error.message === AppErrorCodes.ACTIVITY_TYPE_NOT_FOUND) {
      return NextResponse.json(
        ResBuilder.error(AppErrorCodes.ACTIVITY_TYPE_NOT_FOUND)
          .withDescription("ActivityType not found")
          .build(),
        { status: 404 },
      );
    }
...
export const PUT = async (request: NextRequest, { params }: Props) => {
  try {
    ...
    const { id } = await params;
    const updated = await activityTypeService.updateActivityType(id, validation.data, user);
...
export const DELETE = async (_req: NextRequest, { params }: Props) => {
  try {
    const user = await authenticateUser();
    const { id } = await params;
    await activityTypeService.deleteActivityType(id, user);
    ...
    if (error instanceof Error) {
      ...
      if (error.message === AppErrorCodes.ACTIVITY_TYPE_IN_USE) {
        return NextResponse.json(
          ResBuilder.error(AppErrorCodes.ACTIVITY_TYPE_IN_USE)
            .withDescription("ActivityType is in use")
            .build(),
          { status: 409 },
        );
      }
